...
MachineInstrBuilder
LegalizerHelper::getNeutralElement(unsigned Opcode,
                                   MachineIRBuilder &MIRBuilder, LLT Ty) {
  ...
  case TargetOpcode::G_VECREDUCE_FMINIMUM:
  case TargetOpcode::G_VECREDUCE_FMAXIMUM:
    assert(false && "getNeutralElement unimplemented for G_VECREDUCE_FMINIMUM "
                    "and G_VECREDUCE_FMAXIMUM!");
  }
...
LegalizerHelper::moreElementsVector(MachineInstr &MI, unsigned TypeIdx,
  ...
  case TargetOpcode::G_VECREDUCE_UMIN: {
    ...
    auto NeutralElement =
        getNeutralElement(MI.getOpcode(), MIRBuilder, MoreTy.getElementType());
    for (size_t i = OrigTy.getNumElements(), e = MoreTy.getNumElements();
         i != e; i++) {
      auto Idx = MIRBuilder.buildConstant(LLT::scalar(64), i);
